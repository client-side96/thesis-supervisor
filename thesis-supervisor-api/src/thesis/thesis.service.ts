...
export class ThesisService {
  ...
  async supervisorUpdateThesis(
    ...
    if (isUserSupervisor) {
      ...
      return this.thesisModel.updateOne(
        { _id: thesisId },
        supervisorUpdateThesis,
      );
    } else {
      throw new HttpException(
        'You are not able to update the thesis',
  ...
  async studentSelectThesis(thesisId: string, user: User) {
    ...
    if (!isUserSupervisor) {
      ...
      throw new HttpException(
        'You are not able to select a thesis',
        HttpStatus.FORBIDDEN,
      );
    }
  }
  ...
  async getAllSecondSupervisorTheses(user: User) {
    return new Promise((resolve, reject) => {
      ...
      if (isUserSupervisor) {
        console.log(user);
        this.thesisModel
          .find({ secondSupervisorId: user._id })
          .then((result) => {
            resolve(result);
          })
          .catch((err) => {
